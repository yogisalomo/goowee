#!/usr/bin/env bash
# Build the WASM app + SSR server, serve it, and run the headless-Chrome smoke
# test against it. Requires Go, Node (>=21), and Google Chrome.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
PORT="$PORT" ./bin/ssr-server >/tmp/goowee-e2e-ssr.log 2>&1 &
SRV=$!
